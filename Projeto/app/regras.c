...
int verifica(TAB *jogo, int *p) {
    ...
    for (i = 0; i < jogo -> y; i++) {
         for (j = 0; j < jogo -> x; j++) {
              if (jogo -> tab[i][j].game == '#') { 
                  rRiscadas(j, i, jogo, &vrfcds, &r, p);
                  h = 1;
              }
              if (jogo -> tab[i][j].game >= 'A' && jogo -> tab[i][j].game <= 'Z')
                  rBrancas(j, i, jogo -> tab[i][j].game, jogo, &vrfcds, &r, p);  
        } 
    }
